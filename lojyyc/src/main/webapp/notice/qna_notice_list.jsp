...
<!-- 	</section> -->
	...
	<section id="pageList">
		...
		[이전]&nbsp;
		<%}else{ %>
		<a href="/notice/noticeList.lo?page=<%=nowPage-1 %>" style="color: black;">[이전]</a>&nbsp;
		<%} %>
		...
		[<%=a %>]
		<%}else{ %>
		<a href="/notice/noticeList.lo?page=<%=a %>" style="color: black;">[<%=a %>]
		</a>&nbsp;
		<%} %>
		<%} %>
		...
		[다음]
		<%}else{ %>
		<a href="/notice/noticeList.lo?page=<%=nowPage+1 %>" style="color: black;">[다음]</a>
		<%} %>
